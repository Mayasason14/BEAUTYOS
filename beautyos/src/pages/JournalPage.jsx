  ...
  return (
    <div className="min-h-screen bg-background text-text font-body pb-24">
      ...
      <main className="pt-24 px-6 max-w-3xl mx-auto space-y-10">
        <div>
          <h1 className="font-heading text-h1 text-primary">יומן העור שלי</h1>
          <p className="font-body-md text-on-surface-variant mt-1">מעקב יומי אחר מצב העור, הרגשה ולחות</p>
        </div>

        {/* New entry form */}
        <section className="bg-surface-container rounded-3xl p-8 space-y-6">
          <h2 className="font-heading text-h3 text-primary">רשומה חדשה להיום</h2>

          <div>
            <label className="block font-label-sm text-on-surface-variant mb-3">איך העור שלך מרגיש היום?</label>
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 px-4 py-3 rounded-2xl border transition-all ${
                    mood === m.value
                      ...
                      : 'bg-surface-container-high border-transparent text-on-surface hover:border-outline-variant'
                  }`}
                >
                  <span className="text-2xl">{m.emoji}</span>
                  <span className="font-label-sm">{m.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <label className="block font-label-sm text-on-surface-variant mb-2">
              רמת לחות: <span className="text-primary font-medium">{moistureLevel}%</span>
            </label>
            <input
              type="range"
              min="0"
          ...
          <div>
            <label className="block font-label-sm text-on-surface-variant mb-2">
              רמת אדמומיות: <span className="text-primary font-medium">{rednessLevel} / 10</span>
            </label>
            <input
              type="range"
              min="0"
          ...
          <div>
            <label className="block font-label-sm text-on-surface-variant mb-2">הערות</label>
            <textarea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              rows={3}
              placeholder="מה שמת לב היום? שינויים, תגובות למוצרים..."
              className="w-full bg-transparent border border-outline-variant rounded-2xl p-4 outline-none text-body-md resize-none focus:border-primary transition-colors"
            />
          </div>
        ...
        <section className="space-y-4 pb-8">
          <h2 className="font-heading text-h3 text-primary">הרשומות שלי</h2>
          {loading ? (
            ...
            <p className="text-center text-on-surface-variant py-8">עדיין אין רשומות. התחילי לתעד את מסע העור שלך!</p>
          ) : (
            entries.map(entry => (
              <div key={entry.id} className="bg-surface-container-low rounded-2xl p-6 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="font-label-sm text-on-surface-variant">{formatDate(entry.entry_date)}</span>
                  {entry.mood && (
                    <span className="font-label-sm bg-secondary-container text-on-secondary-container px-3 py-1 rounded-full">
                      {moodMap[entry.mood] ?? entry.mood}
                    </span>
                  )}
                </div>
                {entry.note && (
                  <p className="font-body-md text-on-surface">{entry.note}</p>
                )}
                <div className="flex gap-6 pt-1 border-t border-surface-container">
